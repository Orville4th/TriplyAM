...
class TripLyWindow(QMainWindow):
    ...
    def _show_disclaimer(self):
        ...
        # version string (e.g. "0.4.3") in BOTH the equality check below AND the
        ...
        if self._cfg.get("terms_agreed_version") == "0.4.3":
        ...
        hdr_lbl = QLabel(
            ...
            "<br><span style='color:#888;font-size:12px;'>Open Source Software &nbsp;·&nbsp; v0.4.3 Beta</span>"
        )
        ...
        if result == QDialog.DialogCode.Accepted and chk.isChecked():
            self._agreed_to_terms = True
            self._cfg["terms_agreed_version"] = "0.4.3"
            save_config(self._cfg)
            return True
        return False
    ...
    def _show_whats_new(self):
        ...
        if self._cfg.get("whats_new_shown_version") == "0.4.3":
            return
        # Mark as shown for this version
        self._cfg["whats_new_shown_version"] = "0.4.3"
        save_config(self._cfg)
        ...
        dlg = QDialog(self)
        dlg.setWindowTitle("What's new in TriplyAM 0.4.3")
        dlg.setMinimumWidth(480)
        dlg.setMinimumHeight(400)
        lay = QVBoxLayout(dlg)
        lay.setSpacing(10)

        hdr = QLabel("<b style='font-size:14px;'>What's new in 0.4.3</b>")
        lay.addWidget(hdr)
    ...
    def _dlg_about(self):
        ...
        <h3>0.4.3 — Beta <span class='tag'>current</span></h3>
        <ul>
          <li>Shell-on fixed — mcOffsetMesh returns inverted normals; inner_m is now flipped when volume &lt;= 0 so part - inner gives correct hollow shell</li>
        </ul>
